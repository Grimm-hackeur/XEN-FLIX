...
// ── Servers with auto-fallback ───────────────────────────────
const SERVERS = [
  { label: 'Server 1', key: 'vidsrc' },
  { label: 'Server 2', key: '2embed' },
  { label: 'Server 3', key: 'autoembed' },
  { label: 'Server 4', key: 'multiembed' },
];

function buildUrl(server, type, id, season, episode) {
  switch (server) {
    case 'vidsrc':
      return type === 'movie'
...
export default function DetailPage({ type }) {
  ...
  // Auto-fallback: if iframe doesn't load in 12s, try next server
  useEffect(() => {
    if (!watchMode) return;
    setIframeLoaded(false);
    const timer = setTimeout(() => {
      const idx = SERVERS.findIndex(s => s.key === server);
      if (idx < SERVERS.length - 1) {
        setServer(SERVERS[idx + 1].key);
      }
    }, 12000);
    setAutoFallbackTimer(timer);
    return () => clearTimeout(timer);
  }, [watchMode, server, season, episode]);
  ...
  useEffect(() => {
    if (!watchMode) return;
    const key = `xenflix_continue`;
    const existing = JSON.parse(localStorage.getItem(key) || '[]');
    const entry = { id, type, server, season, episode, ts: Date.now() };
    const updated = [entry, ...existing.filter(e => !(e.id === id && e.type === type))].slice(0, 20);
    localStorage.setItem(key, JSON.stringify(updated));
  }, [watchMode, id, type, server, season, episode]);
  ...
  const totalEps = type === 'tv'
    ? (data.seasons?.find(s => s.season_number === season)?.episode_count || 10)
    : 0;
  const playerUrl = buildUrl(server, type, id, season, episode);
  ...
  return (
    <div className="detail-page">
      ...
      <button className="detail-back" onClick={() => watchMode ? setWatchMode(false) : navigate(-1)}>
        ...
        <div className="watch-container container">
          <div className="watch-header">
            <div>
              ...
              {type === 'tv' && <p className="watch-sub">Season {season} · Episode {episode}</p>}
            </div>
          </div>

          <div className="watch-player">
            {!iframeLoaded && (
              <div className="watch-player__loading">
                <div className="spinner" />
                <p>Loading player…</p>
                <p className="watch-player__hint">If nothing loads, try another server below</p>
              </div>
            )}
            <iframe
              ref={iframeRef}
            ...
            <div className="watch-servers">
              ...
              {SERVERS.map(s => (
                <button
                  ...
                  onClick={() => { setServer(s.key); setIframeLoaded(false); }}
                >
                  {s.label}
                </button>
              ))}
              ...
              <div className="episode-picker__seasons">
                ...
                <div className="episode-picker__season-btns">
                  {Array.from({ length: seasons || 1 }, (_, i) => i + 1).map(s => (
                    <button key={s} className={`server-btn ${season === s ? 'active' : ''}`}
                      onClick={() => { setSeason(s); setEpisode(1); setIframeLoaded(false); }}>
                      S{s}
                    </button>
                  ))}
                </div>
              ...
              <div className="episode-picker__eps">
                ...
                <div className="episode-picker__ep-grid">
                  {Array.from({ length: totalEps }, (_, i) => i + 1).map(ep => (
                    <button key={ep} className={`ep-btn ${episode === ep ? 'active' : ''}`}
                      onClick={() => { setEpisode(ep); setIframeLoaded(false); }}>
                      {ep}
                    </button>
                  ))}
                </div>
          ...
          <div className="detail-hero container">
            ...
            <div className="detail-info">
              <div className="detail-meta-top">
                <span className={`badge ${type === 'tv' ? 'badge-blue' : 'badge-red'}`}>
              ...
              <div className="detail-ratings">
                {rating && (
                  <div className="detail-rating">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style={{color:'var(--accent-gold)'}}>
              ...
              <div className="detail-actions">
                ...
                <button className={`btn btn-ghost ${fav ? 'btn-fav-active' : ''}`} onClick={() => toggleFavorite({ ...data, media_type: type })}>
              ...
              {data.status && (
                <div className="detail-extras">
                  <div className="detail-extra"><span>Status</span><strong>{data.status}</strong></div>
                  ...
                  {cast.map(p => (
                    <div key={p.id} className="cast-card">
                      <div className="cast-card__img">
                        {p.profile_path
                          ? <img src={`${IMAGE_BASE}/w185${p.profile_path}`} alt={p.name} loading="lazy" />
                          : <div className="cast-card__no-img"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>}
                      </div>
                      <p className="cast-card__name">{p.name}</p>
                      <p className="cast-card__role">{p.character}</p>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {similar.length > 0 && (
              <section className="detail-section">
                ...
                <div className="grid-cards">
                  {similar.map((item, i) => <MediaCard key={item.id} item={{ ...item, media_type: type }} index={i} />)}
                </div>
              </section>
            )}
          </div>
